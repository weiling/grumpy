...
//Base object ## This will be the "water" or w/e is at the bottom of the screen when you start
var Base = function() {
	this.height = 5;
	this.width = width;
...
var Player = function() {
	...
	this.width = 68; //of the gc sprtie
	this.height = 88; //of the gc sprite
	...
	this.dir = "right"; //##default direction, always right for gc

	this.x = width / 2 - this.width / 2;
	this.y = height;

	//Function to draw it ##these are dependant on the sprite frames for gc
	this.draw = function() {
		try {
			if (this.dir == "right") {	this.cy = 578; this.cwidth = 136; this.cheight = 176;}
			else if (this.dir == "right_land") {this.cy = 760; this.cwidth = 155; this.cheight = 124;}
...
function Platform() {
	//## these sizes will depend on the sprite frames
	this.width = 70;
	this.height = 17;
	...
	this.draw = function() {
		try {
			...
			else if (this.type == 3 && this.flag === 0){ this.cy = 68; this.cwidth = 128; this.cheight = 80} //before break
			else if (this.type == 3 && this.flag == 1) { this.cy = 160; this.cwidth = 126; this.cheight = 90} //after break
	...
	if (score >= 1000) this.types = [2, 2, 2, 3, 3, 3, 3, 3];
...
//Spring Class
//##this will become the poptart class
var spring = function() {
	this.x = 0;
	this.y = 0;
...
function init() {
	...
	function playerCalc() {
		...
		//##Need to change these to tilt controls, accelerometer (think 360 panorama)
	...
	//Tilt controls
		...
		//##change this to work in landscape.
		else {
			platforms.forEach(function(p, i) {
console.log('p is ', p, 'and i is ', i);
				if (player.vx < 0) {
					p.x -= player.vx;
				}
